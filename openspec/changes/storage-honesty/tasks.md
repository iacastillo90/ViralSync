...
- [ ] **TSH-013 (REFACTOR)** — Title: slice-3 sweep + docs. Verify full suite green; document manual apply for existing dev DBs — `psql "$DATABASE_URL" -c "ALTER TABLE products ADD COLUMN IF NOT EXISTS object_key TEXT;"` (migration note). Rollback: WU-3 boundary. Est: ~5.
