    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
          <p className="text-sm text-slate-400">No hay ideas pendientes de aprobación</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {ideas.map((idea) => {
            const isDone = queuedIds.includes(idea.id);
            return (
              <div key={idea.id} className="bg-slate-900 border border-slate-800 rounded-xl p-5 space-y-4">
                ...
                {isDone ? (
                  <span className="inline-block text-xs bg-emerald-950 text-emerald-300 border border-emerald-500/40 px-2.5 py-1 rounded-full font-semibold">
